...
TsmResult AntColony::FindOptimalPath(Graph& graph) {
  const int number_of_cities = graph.amount_of_vertices();
  ...
  Ant regular_ant(number_of_cities);
...
void AntColony::simulateAnts(
  ...
  for (Ant& ant : ant_colony) {
    ...
    //   std::cout << std::endl;
      ...
      ant.distance += graph.has_edge(current_city, city_to);

      ant.path[step] = city_to;
      current_city = city_to;
    }
    ant.distance += graph.has_edge(ant.path.back(), ant.path.front());
    ant_num++;
...
void AntColony::updatePheromone(std::vector<Ant>& ant_colony,
